...
class SplashController extends GetxController {
  ...
  void _navigateToNext() async {
    ...
    // If role is unknown/unrecognized, fall back to the login screen.
    ...
    // Biometric enabled → show the biometric unlock gate.
    // Biometric NOT enabled → go straight to the role landing (no prompt).
    final biometricEnabled = await StorageHelper.getBiometricEnabled();
    if (biometricEnabled) {
      Get.offNamed(AppRoutes.biometricUnlock);
      return;
    }
