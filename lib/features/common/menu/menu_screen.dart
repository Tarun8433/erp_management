...
//         break;
//       case '/Fees/CollectFees':
//         Get.toNamed(AppRoutes.collectFees);
//         break;
...
/// "Mark Attendance" used to be its own screen/route. It's now merged into
/// the single AttendanceScreen (see attendance_screen.dart), so this entry
/// should never render on its own — whether it's a standalone top-level
/// item or a child under "Attendance".
bool _isMarkAttendanceRoute(String? route) {
  if (route == null) return false;
  final r = route.toLowerCase();
  return r == '/attendence/attendanceregister' ||
      r == '/attendance/attendanceregister';
}

/// Removes "Mark Attendance" items from a menu list (used for the
/// top-level list). Doesn't touch nested subMenus — that's handled
/// separately in _buildDynamicItem so a parent can be "promoted" to a
/// leaf when filtering empties out its children.
List<MenuComponent> _withoutMarkAttendance(List<MenuComponent> items) {
  return items.where((item) => !_isMarkAttendanceRoute(item.route)).toList();
}
...
class MenuScreen extends StatelessWidget {
  ...
  Widget _buildMenuList(
    ...
    // Drop any standalone "Mark Attendance" top-level entry — it's merged
    // into AttendanceScreen now.
    final items = _withoutMarkAttendance(controller.menuItems);
    return ListView.builder(
      padding: const EdgeInsets.symmetric(vertical: 0),
  ...
  Widget _buildDynamicItem(
    ...
    final rawSubMenu = item.subMenu ?? const <MenuComponent>[];
    // Filter out "Mark Attendance" from this item's children.
    final visibleSubMenu = rawSubMenu
        .where((sub) => !_isMarkAttendanceRoute(sub.route))
        .toList();
    final hasChildren = visibleSubMenu.isNotEmpty;
    final color = _palette[paletteIndex % _palette.length];

    if (!hasChildren) {
      // If this used to be a dropdown parent (e.g. "Attendance") whose only
      // real child was "Mark Attendance", it now has no children and no
      // route of its own. Fall back to the first original child's route
      // (typically the real "Attendance" screen) so tapping still works
      // instead of doing nothing.
      final effectiveRoute = (item.route != null && item.route!.isNotEmpty)
          ? item.route
          : (rawSubMenu.isNotEmpty ? rawSubMenu.first.route : null);
  ...
  Widget _buildLeafTile(
    ...
    final textColor = theme.colorScheme.onSurface;
    final iconColor = theme.colorScheme.onSurfaceVariant;
    final effectiveRoute = routeOverride ?? item.route;
    ...
    return SizedBox(
      ...
      child: Material(
        ...
        borderRadius: BorderRadius.circular(12),
        child: InkWell(
          onTap: (effectiveRoute != null && effectiveRoute.isNotEmpty)
              ? () {
                  ZoomDrawer.of(context)?.close();
                  _handleNavigation(effectiveRoute);
                }
              : null,
          borderRadius: BorderRadius.circular(12),
          child: Padding(
  ...
  void _handleNavigation(String route) {
    ...
    switch (route) {
      ...
      // "Mark Attendance" no longer has its own screen — it's merged into
      // AttendanceScreen. Kept as a safety-net mapping in case this route
      // still slips through from the API somewhere; it just opens the
      // same unified Attendance screen now.
      case '/Attendence/AttendanceRegister':
      case '/Attendance/AttendanceRegister':
        Get.toNamed(AppRoutes.attendance);
        break;
      case '/Student/HomeWork':
        Get.toNamed(AppRoutes.homeWork);
