...
class AuthController extends GetxController {
  ...
  final RxString selectedEnvironment = 'Prod'.obs;

  @override
  void onInit() {
    super.onInit();
    _loadEnvironment();
    // MPIN unlock is disabled for now. Biometric unlock is handled by the
    // dedicated BiometricUnlockScreen (routed from splash), so the auth screen
    // is always the plain email/password login.
  }
  ...
  Future<void> _handleLoginSuccess(dynamic result, String token) async {
    ...
    // Register FCM token now that the JWT bearer is in storage.
    final fcmToken = await FirebaseNotificationService.instance.getToken();
    if (fcmToken != null) {
      await FirebaseNotificationService.instance.registerToken(fcmToken);
    }

    // Save full UserDetails from login response for later retrieval
    await StorageHelper.saveUserDetails(UserDetails.fromJson({
      ...
      'branchContactNo': result.branchContactNo,
      'branchLogo': result.branchLogo,
      'isChangePasswordRequired': result.isChangePasswordRequired,
    }));
    ...
    if (result.name != null) {
      ...
      await StorageHelper.saveSchoolName(result.name!);
    }
    if (result.loginName != null && result.loginName!.isNotEmpty) {
      await StorageHelper.saveLoginName(result.loginName!);
    }
